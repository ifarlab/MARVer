...
pubatad = rospy.Publisher(name = 'atad', data_class = adrv, latch = True, queue_size = 1000)
def callbackatad(data):
	global ws, ws_lock
...
pub_dict = { 'atad' : pubatad}
msg_dict = { 'atad' : "adservice/adrv"}
def monitor():
	global pub_error, pub_verdict
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('mon_atad', anonymous=True)
	pub_error = rospy.Publisher(name = 'mon_atad/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'mon_atad/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	rospy.Subscriber('atad_mon', adrv, callbackatad)
