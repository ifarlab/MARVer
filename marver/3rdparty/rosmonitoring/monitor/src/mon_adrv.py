...
pubadrv = rospy.Publisher(name = 'adrv', data_class = adrv, latch = True, queue_size = 1000)
def callbackadrv(data):
	global ws, ws_lock
...
pub_dict = { 'adrv' : pubadrv}
msg_dict = { 'adrv' : "adservice/adrv"}
def monitor():
	global pub_error, pub_verdict
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('mon_adrv', anonymous=True)
	pub_error = rospy.Publisher(name = 'mon_adrv/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'mon_adrv/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	rospy.Subscriber('adrv_mon', adrv, callbackadrv)
