...
puboma = rospy.Publisher(name = 'oma', data_class = oma, latch = True, queue_size = 1000)
def callbackoma(data):
	global ws, ws_lock
...
pub_dict = { 'oma' : puboma}
msg_dict = { 'oma' : "oma_msgs/oma"}
def monitor():
	global pub_error, pub_verdict
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('mon_oma', anonymous=True)
	pub_error = rospy.Publisher(name = 'mon_oma/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'mon_oma/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	rospy.Subscriber('oma_mon', oma, callbackoma)
